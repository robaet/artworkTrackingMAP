...
static bool custom_connection_socket()
{
	...
	char* mUDuRLVal = "http://13.39.16.149:5000/mud/6578616d706c652e636f6d";
	static CK_BYTE info[] =
	{
	 0x65, 0x78, 0x61, 0x6D, 0x70, 0x6C, 0x65, 0x2E, 0x63, 0x6F, 0x6D
	};
	static char mUDuRLValue[64];
	  bool 		result;
      ...
      if (id >= 0) /* no invalid value defined in network library */
      {
		...
		if (com_setsockopt(id, COM_SOL_SOCKET, COM_SO_RCVTIMEO, &timeout, (int32_t)sizeof(timeout)) == COM_SOCKETS_ERR_OK)
		{
	              ...
	              == COM_SOCKETS_ERR_OK)
	          {
                ...
                address.sin_addr.s_addr = COM_HTONL(0x10ab88d9); //
...
//                	PRINT_INFO("Setting the MUD file URL...\n");
//                	rv = csp_installMudURL(mUDuRLVal);
//                	if(rv != CKR_OK)
//                	{
//                		PRINT_INFO("Setting the MUD file URL error  %d\n\r",rv);
//                	}
//                	PRINT_INFO("MUD file URL set.\n");
...
//                						}
                				...
                				{
                					memset(mUDuRLValue,'\0',64);
                					rv = csp_getMuDFileURL(mUDuRLValue);
                					if(rv != CKR_OK)
                					{
                						PRINT_INFO("Get MUDFile URL error  %d\n\r",rv);
 ...
 retry2:
                					...
                					time_begin = HAL_GetTick();
                					ret = com_send(id, (const com_char_t *)mUDuRLValue, buf_snd_len, COM_MSG_WAIT);
                					/* Data send ok ? */
                					if (ret == buf_snd_len)
                					{
                						int32_t total_read_size = 0; /* data can be received in several packets */
