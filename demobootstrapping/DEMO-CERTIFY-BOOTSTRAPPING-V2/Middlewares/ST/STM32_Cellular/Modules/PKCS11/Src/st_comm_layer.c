...
/*This function performs a digital signature operation.*/
...
//
//  com_char_t buf_rsp[MAX_BUFFER_LENGTH_FOR_SIGNATURE];
//  int32_t ret;
//
//  uint8_t charstr[5];
//
...
//        com_char_t  pSendBuffer_set[17] = {0x30, 0x30, 0x32, 0x32, 0x46, 0x31, \
//                                           0x42, 0x36, 0x30, 0x33, 0x38, 0x33, 0x30, 0x31, \
//                                           data[0], data[1], '\0' \
//                                          };
//        /* Transmit to lower level */
//        ret = com_icc_generic_access(h_icc, pSendBuffer_set, 16, buf_rsp, \
...
//              /* answer of SE not proper */
//              /* transform the wrong answer into PKCS11 style */
//              hexstr_to_char((com_char_t *)(buf_rsp), &charstr[0], \
//                             (COM_ICC_NDLC_MIN_RSP_GENERIC_ACCESS_SZ - (uint32_t)1));
//              retValue = (CK_ULONG)(CKR_STM32_CUSTOM_ATTRIBUTE + ((uint32_t)charstr[0] \
//                                                                  * (uint32_t)256) + (uint32_t)charstr[1]);
//            }
//            else
//            {
//              (void)memset(buf_rsp, 0x00, MAX_BUFFER_LENGTH_FOR_SIGNATURE); /* Cleaning Response buffer */
//
//              /*Compute signature*/
...
//              pSendBuffer[1] = 0x30;
//
//              pSendBuffer[2] = 0x32; /* INS */
//              pSendBuffer[3] = 0x41;
//
//              pSendBuffer[4] = 0x39; /* P1 */
//              pSendBuffer[5] = 0x45;
//
//              pSendBuffer[6] = 0x39; /* P2 */
//              pSendBuffer[7] = 0x41;
//
//              (void)convertHexByteToAscii(DataToSignLen, &data[0], &data[1]);
//              pSendBuffer[8] = data[0];
//              pSendBuffer[9] = data[1];
//
